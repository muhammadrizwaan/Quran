    ...
    renderRow = ({ item }) => {
        ...
        return (
            <TouchableOpacity style={{ flexDirection: 'row', justifyContent: 'space-between', borderBottomWidth: 1 }}
                onPress={() => this.props.navigation.navigate('AyahDetail',{ number: item.number, name: item.englishName, translation: item.englishNameTranslation, ayah: item.numberOfAyahs }
                )}>
